...
function processText() {
    ...
    const cvUniqueWords = getUniqueWords(cvWordFrequency, jobDescriptionWordFrequency);
    const jobDescriptionUniqueWords = getUniqueWords(jobDescriptionWordFrequency, cvWordFrequency);

    renderTable(cvWordFrequency, "cvOutput");
    renderTable(jobDescriptionWordFrequency, "jobDescriptionOutput");
    renderTable(cvUniqueWords, "cvUniqueWordsOutput", "Word");
    renderTable(jobDescriptionUniqueWords, "jobDescriptionUniqueWordsOutput", "Word");
}
...
function getUniqueWords(sourceFreq, comparisonFreq) {
    const comparisonWords = new Set(comparisonFreq.map(([word]) => word));
    return sourceFreq.filter(([word]) => !comparisonWords.has(word));
}

function renderTable(data, outputId, firstColumn = "Word") {
    ...
    let tableHTML = `<table><thead><tr><th>${firstColumn}</th><th>Frequency</th></tr></thead><tbody>`;
